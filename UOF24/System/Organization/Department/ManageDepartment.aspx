...
<asp:Content ID="Content1" ContentPlaceHolderID="ContentPlaceHolder1" runat="Server">
    <script type="text/javascript">
        ...
        function ClientNodeDropping(sender, eventArgs) {
            ...
            if (parentNode == null) {
                ...
                var data = [sourceNode.get_value(), sourceNode.get_text(), targetNode.get_value(), sourceNode.get_value(), targetNode.get_value(),displayAllDept,sourceNodeActive];
        ...
        function ClientNodeEditing(sender, eventArgs) {
            ...
            if (escape(text) == '%A0' || escape(text) == "") {
                ...
                if (node.get_value() == null || node.get_value() == "") {
                    ...
                    var data = [guid, text, parentGuid, guid, oldtext,displayAllDept];
                    ...
                    var data = [node.get_value(), text, node.get_value(), oldtext,displayAllDept];
            ...
            if (Key == "createDept") {
                ...
                if (node != null) {
                    ...
                    if (confirm('<%=Ede.Uof.Utility.Page.Common.Resource.GetResource("Common","ConfirmDelete") %>')) {
                        SetToolbarEnable(false);
                        var data = [node.get_value(), level, node.get_value(),displayAllDept];
                ...
                var postionData = [node.get_value(), displayAllDept, "up"];
                movePostion = $uof.pageMethod.sync("GetMovePostion", postionData);

                var data = [node.get_value(), parentNode.get_value(), movePostion, displayAllDept];
                ...
                var index = node.get_index();
                var movePostion = index;
                var postionData = [node.get_value(), displayAllDept, "down"];
                movePostion = $uof.pageMethod.sync("GetMovePostion", postionData);
                ...
                if (confirm('<%=lblConfirmEnable.Text %>')) {
                    ...
                    var enableType = true;
                    $("#<%=hiddenNodeId.ClientID%>").val(node.get_value());
                    var data = [node.get_value(), enableType, displayAllDept,needVaild];
           ...
           <asp:UpdatePanel ID="UpdatePanel1" runat="server" UpdateMode="Conditional">
                <ContentTemplate>
                    ...
                    <telerik:RadTreeView ID="RadTreeView1" runat="server" AllowNodeEditing="true" EnableDragAndDrop="true" OnClientNodeClicking="ClientNodeClicking"
                        OnClientNodeClicked="ClientNodeClicked" Width="594px" Height="459px"
                        OnClientNodeEditing="ClientNodeEditing" OnClientNodeDragStart="ClientNodeDragStart"
                        OnClientNodeDropping="ClientNodeDropping" EnableNodeTextHtmlEncoding="True">
                    </telerik:RadTreeView>
    ...
    <asp:Label ID="lbNeedItemSelected" runat="server" Text="請先選擇部門" Visible="False"
    ...
    <asp:Label ID="lbNotAllowDelete2" runat="server" Text="部門內仍有子部門/人員存在，不可刪除！" Visible="False"
