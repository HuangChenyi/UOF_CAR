...
<script type="text/javascript">
    function CheckTimeOff(e, args) {
        ...
        if (otNbr.val() == '') return;
        var data = [$('#<%= hfTaskID.ClientID  %>').val(), $('#<%= hfApplicantGuid.ClientID  %>').val()];  var result = $uof.pageMethod.syncUc("EIP/Duty/WKFOptionalFields/OverTimeCancelInfoUCV2.ascx", "CheckTimeOff", data);
        if (result == "true") {
            args.IsValid = false;
        }
    }
            ...
            <tr>
                <td>
                    <table class="PopTable" cellspacing="0" width="100%">
                        ...
                        <tr id="placeTr" runat="server" style="white-space: nowrap;">
                            <td>
