...
<script type="text/javascript">
    ...
    function CheckSelectLeaveCode(source, args) {
        var leaCode = $("#<%=hfApplicantEmployeeNo.ClientID%>").val();
        var askId = $("#<%=lblAskId.ClientID %>").text(); //表單代碼
        //沒有公司統編或員編就不跑其他驗證
        var applicantEmployeeNo = $("#<%=hfApplicantEmployeeNo.ClientID%>").val();
        var applicantCompanyNo = $("#<%=hfApplicantCompanyUnifiedId.ClientID%>").val();
...
<asp:UpdatePanel ID="UpdatePanel1" runat="server">
    <ContentTemplate>
        <table class="LeaveCancelTable104">
            ...
            <tr>
                <td>
                    <asp:Label ID="Label5" runat="server" Text="請假時時(天)數" meta:resourcekey="Label5Resource1"></asp:Label>
                </td>
            </tr>
            <tr>
                <td>
        ...
        <table>
            ...
            <tr>
                <td>
                    <asp:CustomValidator ID="cvCheckDeleteLeaveData" Display="Dynamic" runat="server" ErrorMessage="CustomValidator" EnableTheming="True" meta:resourcekey="cvCheckDeleteLeaveDataResource1"></asp:CustomValidator>
                </td>
            </tr>
        </table>
